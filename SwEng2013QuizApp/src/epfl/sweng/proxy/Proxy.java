...
public final class Proxy extends EventEmitter implements IServer, EventListener {
    ...
    private void serializeQuestion(QuizQuestion question) throws IOException {
        try {

            File questionBackup = new File(BACKUP_FILE_NAME);

            JSONObject rootObject = null;

            if (questionBackup.exists()) {

                InputStream is = context.openFileInput(BACKUP_FILE_NAME);
                ...
                if (is != null) {
                    ...
                    if (rootObject.isNull("questions")) {
                        rootObject.put("questions", new JSONArray());
                    }
                }


            } else {
                if (questionBackup.createNewFile()) {
            ...
            if (rootObject != null) {
                rootObject.getJSONArray("questions").put(new JSONObject(question.toJSON()));

                OutputStreamWriter writer = null;

                writer = new OutputStreamWriter(context.openFileOutput(BACKUP_FILE_NAME, Context.MODE_PRIVATE));
    ...
    public Iterator<QuizQuestion> readPendingQuizQuestion() throws IOException {
        try {
            InputStream is = context.openFileInput(BACKUP_FILE_NAME);
            StringBuffer buffer = new StringBuffer();
            BufferedReader reader = new BufferedReader(new InputStreamReader(is));
            ...
            if (!rootObject.isNull("questions")) {
                JSONArray questionsInJson = rootObject.getJSONArray("questions");
                List<QuizQuestion> questionsList = new LinkedList<QuizQuestion>();
                for (int i = 0; i < questionsInJson.length(); i++) {
                    questionsList.add(new QuizQuestion(questionsInJson.getJSONObject(i).toString()));
                }
        ...
        return new LinkedList<QuizQuestion>().iterator();
    }
}
