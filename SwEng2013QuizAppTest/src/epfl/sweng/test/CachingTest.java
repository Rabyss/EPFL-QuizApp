...
public class CachingTest extends ActivityInstrumentationTestCase2<MainActivity> {
	...
	public void testCacheInAction() {
		...
		QuizQuestion displayedQuiz = ((ShowQuestionsActivity) solo
				.getCurrentActivity()).getCurrentQuizQuestion();
	...
	/*
	 * Need to change the exception thrown in the mock to make it work... public
	 * void testWhenNoInternetAccess() { login(); QuizQuestion cachedQuiz =
	 * cacheAQuestion();
	 * 
	 * solo.clickOnButton(mRandomQuestionButton);
	 * getActivityAndWaitFor(TTChecks.QUESTION_SHOWN); QuizQuestion
	 * displayedQuiz = ((ShowQuestionsActivity)
	 * solo.getCurrentActivity()).getCurrentQuizQuestion();
	 * compareQuizQuestion(cachedQuiz, displayedQuiz); }
	 */
	private void compareQuizQuestion(QuizQuestion cachedQuiz,
			QuizQuestion displayedQuiz) {
		assertEquals("Cached question body and displayed are not the same.",
				cachedQuiz.getQuestion(), displayedQuiz.getQuestion());

		int numCachedAnswers = cachedQuiz.getAnswers().size();
		assertEquals(
				"Cached question has not the same number of answers as the displayed one.",
		...
		for (int i = 0; i < numCachedAnswers; i++) {
			assertEquals("Cached answer is not the same as the displayed one.",
					cachedQuiz.getAnswers().get(i), displayedQuiz.getAnswers()
							.get(i));
			assertEquals("Not the same right answers.",
					cachedQuiz.isSolution(i), displayedQuiz.isSolution(i));
		}

		assertEquals(
				"Cached question has not the same number of tags as the displayed one.",
				cachedQuiz.getTags().size(), displayedQuiz.getTags().size());

		assertTrue(
				"Tags are not the same between the cached and the displayed quiz.",
	...
	private QuizQuestion cacheAQuestion() {
		...
		solo.clickOnButton(mRandomQuestionButton);
		getActivityAndWaitFor(TTChecks.QUESTION_SHOWN);
		QuizQuestion cachedQuiz = ((ShowQuestionsActivity) solo
				.getCurrentActivity()).getCurrentQuizQuestion();
	...
	private void setOfflineMode() {
		...
		CheckBox cBox = (CheckBox) getActivity().findViewById(
				R.id.onlineModeCheckBox);
		assertFalse("Cannot find the checkbox", cBox == null);
		System.out.println(cBox.isChecked());
		if (!cBox.isChecked()) {
			solo.clickOnView(cBox); // check the offline-mode checkbox if
									// unchecked
			getActivityAndWaitFor(TTChecks.OFFLINE_CHECKBOX_ENABLED);
		}

		assertFalse(
				"The app must be in offline mode after checking the check box.",
				AppContext.getContext().isOnline());
	}
