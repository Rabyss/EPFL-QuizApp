...
public class EditQuestionActivityTest extends
	...
	private static final String FETCHING_ERROR_MESSAGE = "Could not upload the question"+
															"to the server, switched to offline mode.";
	...
	public void testSubmit() {
		...
		assertTrue("+ button cannot be found.",
				solo.searchButton(PLUS_BUTTON_TEXT));
		solo.clickOnButton(PLUS_BUTTON_TEXT);
		waitForChange();

		fillNextAnswerBody(scdAnswerBody);

		assertTrue(FALSE_ANSWER_BUTTON + " buttons cannot be found.",
				solo.searchButton(FALSE_ANSWER_BUTTON, 2));
		solo.clickOnButton(FALSE_ANSWER_BUTTON);

		assertTrue("Tags editor cannot be found.",
		...
		assertTrue("Submit success msg found",
				solo.searchText(FETCHING_SUCCESS_MESSAGE));
		assertFalse("Error message found.",
				solo.searchText(FETCHING_ERROR_MESSAGE));
		

		assertFalse("Question body has changed after bad submit.",
				solo.searchEditText(questionBody));
		assertFalse("First answer body has changed after bad submit.",
				solo.searchEditText(firstAnswerBody));
		assertFalse("Second answer body has changed after bad submit.",
				solo.searchEditText(scdAnswerBody));
		assertFalse("Tags body has changed after bad submit.",
				solo.searchEditText(tags));
		assertFalse(
				"The checked answer does no longer exist after bad submit.",
	...
	public void testBadRequest() {
		...
		getActivityAndWaitFor(TTChecks.NEW_QUESTION_SUBMITTED);
		assertTrue(
				"Client error not notified.",
				solo.searchText(getActivity().getString(
						epfl.sweng.R.string.submit_server_failure)));
	...
	public void testTags() {
		...
		assertTrue(FALSE_ANSWER_BUTTON + " button cannot be found.",
				solo.searchButton(FALSE_ANSWER_BUTTON));
		solo.clickOnButton(FALSE_ANSWER_BUTTON);
		...
		assertFalse(
				"Submit button should not be enabled when tags are just spaces.",
				submitButton.isEnabled());
		solo.clearEditText(tagsEditor);

		solo.typeText(tagsEditor, "\t");
		assertFalse(
				"Submit button should not be enabled when tags are just tabs.",
				submitButton.isEnabled());
		solo.clearEditText(tagsEditor);

		solo.typeText(tagsEditor, "\n\n\n");
		assertFalse(
				"Submit button should not be enabled when tags are just \\n.",
				submitButton.isEnabled());
		solo.clearEditText(tagsEditor);

		solo.typeText(tagsEditor, "\t\n\n   ");
		assertFalse(
				"Submit button should not be enabled when tags are just spaces of different sorts.",
				submitButton.isEnabled());
		solo.clearEditText(tagsEditor);

		solo.typeText(tagsEditor, " , ");
		assertFalse(
				"Submit button should not be enabled when tags are just spaces and commas.",
				submitButton.isEnabled());
		solo.clearEditText(tagsEditor);

		solo.typeText(tagsEditor, "a b");
		assertTrue("Submit button should be enabled when tags are valid ones.",
				submitButton.isEnabled());
		solo.clearEditText(tagsEditor);
	}

	public void testRemovingAllAnswers() {
		assertTrue("No button to delete the answer can be found.",
				solo.searchButton(MINUS_BUTTON_TEXT, 1));
	...
	public void testCheckAndUncheck() {
		...
		assertFalse("A question is already checked at first.",
				solo.searchButton(TRUE_ANSWER_BUTTON_TEXT));
		assertTrue("No button to check the answer exists.",
				solo.searchButton(FALSE_ANSWER_BUTTON));
		...
		solo.clickOnButton(PLUS_BUTTON_TEXT);
		waitForChange();
		assertTrue("The + button does not add answer as expected.",
				solo.searchText(ANSWER_EDITOR_TEXT, THREE));

		solo.clickOnButton(FALSE_ANSWER_BUTTON);
		waitForChange();
		assertTrue("No checked question after having checked one.",
				solo.searchButton(TRUE_ANSWER_BUTTON_TEXT));

		solo.clickOnButton(FALSE_ANSWER_BUTTON);
		assertFalse("Two question can be checked at the same time.",
				solo.searchButton(TRUE_ANSWER_BUTTON_TEXT, 2));
	...
	public void testSubmitButton() throws InterruptedException {
		...
		Button submitButton = solo.getButton(SUBMIT_BUTTON_TEXT);
		assertFalse("Submit button must be disabled at first.",
				submitButton.isEnabled());

		// Fill the question and make sure the submit button is still inactive
		fillQuestionBody("Quesiton body");
		assertFalse(
				"Editing the question is not sufficient to get the submit button active.",
				submitButton.isEnabled());

		// Fill only one answer and make sure the button is still inactive
		assertTrue("No answer editor can be found at first.",
				solo.searchEditText(ANSWER_EDITOR_TEXT));
		fillNextAnswerBody("First answer body");

		// Fill a second answer and make sure that the button is still inactive
		// because no tags
		assertTrue("+ button cannot be found.",
				solo.searchButton(PLUS_BUTTON_TEXT));
		solo.clickOnButton(PLUS_BUTTON_TEXT);
		...
		assertTrue(
				"No new answer editor can be found after pressing the + button.",
				solo.searchEditText(ANSWER_EDITOR_TEXT));
		String secondAnswerBody = "Second answer body";
		fillNextAnswerBody(secondAnswerBody);
		assertFalse("The submit button can become enabled with no tags",
				submitButton.isEnabled());
		...
		solo.typeText(tagsEditor, "      ");
		assertFalse(
				"The submit button can become enabled with only spaces as tags.",
				submitButton.isEnabled());

		solo.typeText(tagsEditor, "tag1 tag2");
		assertFalse(
				"The submit button can become enabled when no answer is marked as true.",
				submitButton.isEnabled());

		assertTrue(FALSE_ANSWER_BUTTON + " cannot be found.",
				solo.searchButton(FALSE_ANSWER_BUTTON));
		solo.clickOnButton(FALSE_ANSWER_BUTTON);
		waitForChange();
		assertTrue("The submit button does not become enabled when it should.",
				submitButton.isEnabled());

		clearEditText(secondAnswerBody);
		assertFalse(
				"The submit button can become enabled with a empty answer.",
				submitButton.isEnabled());
	}
	...
	private void clearEditText(String currentBody) {
		assertTrue(
				"No editor can be found with the body '" + currentBody + "'",
				solo.searchEditText(currentBody));
